...
describe('OVM_GasPriceOracle', () => {
  ...
  describe('getL1Fee', async () => {
    for (const input of inputs) {
      it(`case: ${input}`, async () => {
        ...
        const expected = calculateL1Cost(input, overhead).mul(l1BaseFee)
        expect(cost).to.deep.equal(expected)
      })
    }
  })
  ...
  describe('get scalar', () => {
    it('should return 0 at first', async () => {
      expect(await OVM_GasPriceOracle.scalar()).to.equal(0)
    })
